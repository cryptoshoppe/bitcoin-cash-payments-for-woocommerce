...
{
    $source_url	=	"https://XXXapiv2.bitcoinaverage.com/indices/global/ticker/short?crypto=BCH&fiat={$currency_code}";
...
{
    $source_url =       "https://api.blockchair.com/bitcoin-cash/stats";
    $result = @BWWC__file_get_contents($source_url, false, $bwwc_settings['exchange_rate_api_timeout_secs']);
...
function BWWC__is_gateway_valid_for_use(&$ret_reason_message=null)
{
    ...
    if ($bwwc_settings['service_provider']=='electrum_wallet') {
        ...
        if (!$mpk) {
            $reason_message = __("Please specify Electron Cash  Master Public Key (MPK). <br />To retrieve MPK: launch your Electron Cash wallet, select: Wallet->Information", 'woocommerce');
            $valid = false;
